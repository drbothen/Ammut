...
from models.schemas import TargetProfile
from services.base import AbstractParameterOptimizerService


class ParameterOptimizerService(AbstractParameterOptimizerService):
    async def optimize_parameters(
        ...
        optimizer_method = getattr(self, f"_optimize_{tool.replace('-', '_')}_params", None)

        if optimizer_method and callable(optimizer_method):
            return optimizer_method(profile, context)

        # Fallback for unknown tools or if a specific optimizer is not implemented
        return {"target": profile.target, "error": f"No optimization strategy for tool: {tool}"}
    ...
    def _optimize_kube_hunter_params(self, profile: TargetProfile, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        params = {"target": profile.target, "report": "json"}
    ...
    def _optimize_trivy_params(self, profile: TargetProfile, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        params = {"target": profile.target, "severity": "HIGH,CRITICAL"}
        if profile.target_type == TargetType.CONTAINER_IMAGE:
            params["scan_type"] = "image"
        elif profile.target_type == TargetType.IAC_CODE:
            params["scan_type"] = "config"
        return params
